...
function generateHistogram(histogramData) {
  ...
  histogramContainer.innerHTML = ""; // Очищаем предыдущее содержимое.

  if (histogramData.length > 0) {
    let histogramHtml = "<h3>Явно проявленные паттерны</h3><div class='histogram-bar-container'>";

    histogramData.forEach(({ pattern, percentage }) => {
      histogramHtml += `
...
function generatePDF(resultsData, patternsData) {
  ...
  const generateContent = (results, userFullName, testDate) => {
        ...
        categoryData.subcategories.forEach((subcategory) => {
          if (subcategory.responses.length > 0) {
            ...
            if (dominanceResponse.status !== "Нейтрально") {
              console.log(`${dominanceResponse.status} ${dominanceResponse.pattern}`); // Выводим статус и паттерн
    ...
    content.push({
      ...
      margin: [0, 20, 0, 0], // Установка отступов
    });
  ...
  const generateDominantPatternsContent = (dominantPatterns, enrichedData) => {
    ...
    dominantPatterns.forEach((item) => {
      ...
      if (percentage > 74) {
        ...
        if (category) {
          const subcategory = category.subcategories.find((subcat) => subcat.patterns.some((pattern) => pattern.title === patternLabel));
        ...
        const color = category ? category.color : "#a3cfff";
    ...
    if (dominantPatternsColumns.length > 0) {
      content.push({
      ...
      if (abbreviations.length > 0) {
        ...
        content.push({
          columns: [
            { width: "*", stack: descriptionItemsLeft },
            { width: "*", stack: descriptionItemsRight },
          ],
          columnGap: 10,
        });
      }
  ...
  const docDefinition = {
    content: [
      ...
      {
        text: [{ text: "По лю­бым возни­кающим вопро­сам можно свя­заться с авто­ром проек­та Еленой Семеновой:", style: "secondText" }],
        margin: [0, 0, 0, 10],
      },
      {
        text: [
          { text: "email Еленой Семеновой: ", style: "secondTextLink" },
      ...
      },
      {
        text: [
          ...
          { text: "@SemenovaElena", style: "secondLink", link: "@SemenovaElena" },
        ],
        margin: [0, 0, 0, 5],
      },
      {
        text: [
          ...
          { text: "@life_watch", style: "secondLink", link: "@life_watch" },
        ],
        margin: [0, 0, 0, 5],
      },
      {
      ...
      {
        text: [
          { text: "Пройти тест заново: ", style: "secondTextLink" },
