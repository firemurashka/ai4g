...
function showResults() {
  ...
  testInfo.style.maxWidth = "1200px";
...
async function loadPatterns() {
  try {
    ...
    return (await response.json()).categories || []; // Возвращаем список категорий
  } catch (error) {
...
function getDominanceResponse(subcategory) {
  // Находим максимальный процент из ответов в подкатегории
  const maxPercentage = Math.max(...subcategory.responses.map((res) => res.percentage));

  // Фильтруем ответы, чтобы получить только те, что соответствуют максимальному проценту
  ...
  // Проверкa на количество доминирующих паттернов
  if (dominantResponses.length > 1) {
    // Если есть более одного паттерна с максимальным процентом, проверяем их значение
    if (maxPercentage >= 50) {
      dominanceLabel = "Нейтрально"; // Исправлено на Нейтрально
    } else {
    ...
    // Определение статуса и извлечение названия паттерна
    if (percentage < 60) {
      dominanceLabel = "Умеренно:"; // Замените на нужный вам статус
    } else {
      dominanceLabel = "Явно:"; // Замените на нужный вам статус
    }

    dominantPattern = dominantResponse.pattern; // Название паттерна
  }

  // Возвращаем объект с результатами
  return {
    status: dominanceLabel,
...
function generatePDF(resultsData, patternsData) {
  ...
  const generateContent = (results, userFullName, testDate) => {
    ...
    for (const category in results) {
      ...
      if (categoryData && Array.isArray(categoryData.subcategories)) {
        categoryData.subcategories.forEach((subcategory) => {
          const patterns = subcategory.patterns || []; // Убедитесь, что patterns является массивом
          if (patterns.length > 0) {
            patternCategories[category] = patternCategories[category] || [];
            patternCategories[category].push({
              subcategoryTitle: subcategory.subcategory || "", // Исправлено на subcategory.subcategory
              patterns: patterns.map((pattern) => ({
                title: pattern.title || "Без названия", // Обработка отсутствующего названия паттерна
                description: pattern.description || "Нет описания паттерна", // Обработка отсутствия описания паттерна
              })),
            });
          }
        });
  ...
  const generateDominantPatternsContent = (dominantPatterns, enrichedData) => {
    const content = [
      {
        text: "Доминирующие паттерны",
        style: "dominantTitle",
      },
    ];

    const dominantPatternsColumns = [];
    const abbreviations = []; // Массив для хранения расшифровок

    dominantPatterns.forEach((item) => {
      const patternLabel = item.pattern;
      const percentage = item.percentage;

      // Генерация аббревиатуры из первых букв
      const abbreviation = patternLabel
        .split(" ")
        .map((word) => word[0])
        .join("")
        .toUpperCase();

      // Добавляем расшифровку к массиву
      abbreviations.push({ abbreviation, patternLabel });

      // Получаем цвет категории, к которой относится данный паттерн
      const category = Object.values(enrichedData).find((cat) =>
        cat.subcategories.some(
          (subcat) => subcat.patterns.some((pattern) => pattern.title === patternLabel) // изменяем на title
        )
      );
      const color = category ? category.color : "#a3cfff"; // Используем цвет категории или стандартный
      ...
      const pctData = [
        ...
        // Вертикальная линия
        {
          stack: [
            ...
            // Активная часть линии с цветом категории
            {
              canvas: [
                {
                  type: "rect",
                  x: 0,
                  y: 100 - percentage, // Позиционируем по проценту
                  w: 8, // Ширина линии
                  h: percentage,
                  color: color, // Используем цвет категории
                  r: 5,
                },
              ],
              margin: [0, -100, 0, 0], // Смещение для наложения
            },
          ],
          margin: [0, 0, 0, 5], // Уменьшаем отступы между колонками
        },

        // Аббревиатура снизу
        {
          text: abbreviation,
          style: "abbreviationText",
        },
      ];

      dominantPatternsColumns.push({
        width: "*", // Делает ширину колонки динамической
        stack: pctData,
        margin: [0, 0, 0, 5], // Уменьшаем отступы между колонками
      });
    });
    ...
    content.push({
      ...
      columnGap: 5, // Уменьшаем пробел между колонками
      alignment: "center",
    });

    // Добавление блока расшифровки аббревиатур в два столбца
    if (abbreviations.length > 0) {
      ...
      // Разделяем аббревиатуры на два массива для двух столбцов
      const halfLength = Math.ceil(abbreviations.length / 2);
      const leftColumn = abbreviations.slice(0, halfLength);
      const rightColumn = abbreviations.slice(halfLength);

      const descriptionItemsLeft = leftColumn.map((item) => {
        return {
          text: `${item.abbreviation}: ${item.patternLabel}`,
          style: "abbreviationLabel",
        };
      });

      const descriptionItemsRight = rightColumn.map((item) => {
        return {
          text: `${item.abbreviation}: ${item.patternLabel}`,
          style: "abbreviationLabel",
        };
      ...
      // Добавляем столбцы в контент
      content.push({
        columns: [
          {
            width: "*",
            stack: descriptionItemsLeft,
          },
          {
            width: "*",
            stack: descriptionItemsRight,
          },
        ],
        columnGap: 10, // Пробел между столбцами
      });
    }
